...
const overlayedCanvas: HTMLCanvasElement = document.getElementById(
  'overlayed-canvas'
...
const addShape: TypeAddShape = (
  ...
  if (!returnValue)
    Particles.push(new (helpers.getRandomItem(shapeArray))(ctx, attr) as Shape);
...
target.addEventListener('mousemove', (evt: MouseEvent) =>
  eventMove({ x: evt.pageX, y: evt.pageY })
);
target.addEventListener('mousedown', (evt: MouseEvent) =>
  eventDown({ x: evt.pageX, y: evt.pageY })
);
target.addEventListener('mouseup', (evt: MouseEvent) =>
  eventUp({ x: evt.pageX, y: evt.pageY })
);
